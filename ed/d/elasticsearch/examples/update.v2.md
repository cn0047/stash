...
# another example
curl -XPOST "$url/2/_update?pretty" -d '{
...
curl -XPOST "$url/_update_by_query?conflicts=proceed&pretty" -d '{
    "query": {"match_all" : {}},
    "script" : {
...
curl -XPOST "$url/_update_by_query&scroll_size=1000" -d '{
  "script": {
